#pragma once
#ifndef LEETCODE_318_H
#define LEETCODE_318_H
    ...
    public:
        int maxProduct(std::vector<std::string>& words)
        {
            ...
            for (const auto& word: words)
            {
                ...
                if (mask_map.find(mask) == mask_map.end()) mask_map[mask] = word_len;
                else mask_map[mask] = std::max(mask_map[mask], word_len);
            }
